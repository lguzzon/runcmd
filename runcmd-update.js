...
async function main() {
  try {
    if (!fs.existsSync(RUNCMD_HOME)) fs.mkdirSync(RUNCMD_HOME, { recursive: true });
    ...
    const res = await fetch(UPDATE_URL_BASE + '/version.txt', { signal: AbortSignal.timeout(5000) });
    ...
    console.error('[INFO] New version available: ' + remoteVersion + '. Updating...');
    const scriptRes = await fetch(UPDATE_URL_BASE + '/runcmd.bat', { signal: AbortSignal.timeout(10000) });
    if (!scriptRes.ok) throw new Error('Failed to fetch update');
    const newContent = await scriptRes.text();

    if (!newContent.includes('runcmd.bat')) throw new Error('Invalid update content');
