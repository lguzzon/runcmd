...
export function runGit(args, { allowFail = false, dryRun = false, pipeStdout = false } = {}) {
  const cmd = `git ${args}`;
  if (dryRun) {
    logInfo(`[dry-run] ${cmd}`);
    return "";
...
export function pullBranch(branch, { dryRun = false, offline = false, allowFail = true } = {}) {
  if (offline) return;
...
export function runGitFlow(args, { allowFail = false, dryRun = false, pipeStdout = false } = {}) {
...
export function getGitFlowConfig() {
  const master = runGit("config --get gitflow.branch.master", { allowFail: true });
...
export function ensureGitFlowAvailable({ autoInstall, offline, dryRun }) {
  ...
  if (!autoInstall) {
    logWarn("git-flow not found. Re-run with --auto-install to install it (may require sudo).");
    return false;
  }
  if (dryRun) {
...
function parseArgs(argv) {
  ...
  if (
    command &&
    ["release", "hotfix", "start", "finish", "publish", "track", "delete"].includes(command)
  ) {
...
function handleInstall(opts) {
  ...
  if (!available) return;
  const inRepo = runGit("rev-parse --is-inside-work-tree", { allowFail: true }) === "true";
  if (inRepo) {
